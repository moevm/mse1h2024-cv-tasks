...
Передача токенов и ключей в Act для запуска GitHub Actions происходит напрямую через указание файла, содержащего секреты. Синтаксис файла .secrets совпадает с синтаксисом файлов, содержащих переменные окружения. В данном файле должны содержать следующие переменные: `GITHUB_TOKEN`, `PUBLIC_SSH_KEY`, `PRIVATE_SSH_KEY`.  
